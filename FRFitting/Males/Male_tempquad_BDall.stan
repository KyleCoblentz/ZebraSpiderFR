// Stan model to fit FR with temperature dependence and seperate interference parameters
...
generated quantities {
  vector[N] log_lik;
  for (n in 1:N){
    log_lik[n] = binomial_lpmf(y[n] | n_t[n], (a[n]*R[n])/(1 + a[n]*d[n]*R[n] + g*C[n] + gF*F[n] + gJ*J[n]) * d[n]);
  }
}
