// Stan model to fit FR with no temp dependence, sep interference for each sex/stage
//
...
generated quantities {
  vector[N] log_lik;
  for (n in 1:N){
    log_lik[n] = binomial_lpmf(y[n] | n_t[n], (a*R[n])/(1 + a*d[n]*R[n] + g*C[n] + gM*M[n] + gJ*J[n]) * d[n]);
  }
}
